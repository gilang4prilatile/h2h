...
class InboundBC20Export implements FromCollection,WithHeadings
{
    use Exportable;
        ...
        foreach($inbound as $inbound){
            ...
            // include('BC23_get_data.php');
                ...
                array_push($myarray, [
                    ...
                    'Importir'                  => $inbound->inboundImportir->profile->name ?? '',
                    'Pemilik Barang'            => $inbound->inboundPemilikBarang->profile->name ?? '',
                    'PPJK'                      => $inbound->inboundPpjk->profile->name ?? '',
                    'Penjual Barang'            => $inbound->inboundPenjualBarang->profile->name ?? '',
                    'Pengirim Barang'           => $inbound->inboundPengirimBarang->profile->name ?? '',
                    'Pemusatan'                 => $inbound->inboundPemusatanBarang->profile->name ?? '',
