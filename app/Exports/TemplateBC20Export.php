...
class TemplateBC20Export implements WithMultipleSheets
{
    use Exportable;
    ...
    public function sheets(): array
    {
        ...
        foreach($keys as $key){
            ...
            if($key == 'HEADER'){
                ...
                // Fetching unload, load, transit, and tujuan ports
                ...
                // Fetching unload and load ports safely
                $unloadport = $unload ? DB::table('ports')->where('id', '=', $unload->port_id)->first() : null;
                $loadport   = $load ? DB::table('ports')->where('id', '=', $load->port_id)->first() : null;

                // Check if $transit is not null before accessing port_id
                if ($transit && $transit->port_id) {
                    $transitport = DB::table('ports')->where('id', '=', $transit->port_id)->first();
                } else {
                    $transitport = null;
                }
                
                $tujuanport    = $tujuan ? DB::table('ports')->where('id','=',$tujuan->port_id)->first() : null;
