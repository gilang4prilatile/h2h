...
<script>
    ...
    $(document).ready(function() {
            ...
            "columnDefs": [
                {
                    ...
                    "render": function (val, type, data, meta) { // Note: not sure what "val" is actually
                        return (
                            '<div class="btn-group">' +
                                '<button type="button" class="btn btn-warning btn-sm" onclick="editPemilikBarangById('+data['id']+')">Edit</button>'+
                                '<button type="button" class="btn btn-danger btn-sm" onclick="deletePemilikBarangById('+data['id']+')"">Delete</button>'+
                            '</div>'
                        );
                    },
                },
        ...
        deletePemilikBarangById = (id) => {
            ...
            $.ajax({
                type: "GET",
                url: `{{url('/')}}/inbound-profile/delete/${id}`,
        ...
        editPemilikBarangById = (id) => {
            let selectedData = banyakPemilikBarang.find(function (val) {return val.id == id})
