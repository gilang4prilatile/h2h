        ...
        <div id="kt_content_container" class="container">
            ...
            <div class="card">
                ...
                <div class="card-body pt-0" style="margin-top: 20px">
                    <!--begin:Form-->
                    <form action="{{url('master-data/good-conversions/update')}}/{{$row_goods->id}}" method="post" id="form-good-conversions">
                        {{csrf_field()}}
                        <div class="form-group row">
                            <div class="col-md-3">
                                <label class="form-label required">Code</label>
                                <input type="text" class="form-control " name="details[kode_barang]" value="{{$row_goods_detail['kode_barang']}}" required>
                            </div>

                            <div class="col-md-3">
                                <label class="form-label required">Name</label>
                                <input type="text" class="form-control " name="name" value="{{$row_goods->name}}" required>
                            </div>
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="uom_id" id="uom_id" class="form-select  detail-inputs form-control-solid" required>
                                    ...
                                    <option value="{{$row->id}}" {{$row_goods->uom_id==$row->id?"selected":""}}>{{$row->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                                <label class="form-label required">Uraian barang</label>
                                <input type="text" class="form-control " name="details[uraian]" value="{{$row_goods_detail['uraian']}}" required>
                            </div>
                        </div>
                        <div class="form-group row" style="margin-top: 20px;">
                            {{-- <div class="col-md-4">
                        <label class="form-label required">Amount</label>
                        <input type="number" class="form-control " name="amount" step="any"  value="{{$row_goods->amount}}" required>
                        </div> --}}
                        <div class="col-md-4">
                            <label class="form-label required">Nett Weight</label>
                            <div class="input-group mb-5">
                                <input type="number" class="form-control " name="nett_weight" step="any" value="{{ floatVal($row_goods->nett_weight) }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="input-group mb-5">
                                <input type="number" class="form-control " name="volume" step="any" value="{{ abs($row_goods->volume) }}" required>
                                <span class="input-group-text" id="">m3</span>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label required">Merk</label>
                            <input type="text" class="form-control " name="details[merk]" value="{{$row_goods_detail['merk']}}" required>
                        </div>
                </div>
                <div class="form-group row mt-3">
                    <div class="col-md-4">
                        <label class="form-label required">Tipe</label>
                        <input type="text" class="form-control " name="details[type]" value="{{$row_goods_detail['type']}}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label required">Ukuran</label>
                        <input type="text" class="form-control " name="details[ukuran]" value="{{$row_goods_detail['ukuran']}}" required>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label required">Spesifikasi Lain</label>
                        <input type="text" class="form-control " name="details[spesifikasi_lain]" value="{{$row_goods_detail['spesifikasi_lain']}}" required>
                    </div>
                </div>
                <hr class="mt-8">
                <div class="row mt-3">
                ...
                <div class="text-center mt-10">
                    <button type="submit" class="btn btn-primary" id="btn-submit" style="width: 25%">Submit</button>
                </div>
                <!--end::Actions-->
                <!--end:Form-->
                </form>
...
<!--end::Content-->
@include('master-data.good-conversions.popup-goods-update')
@endsection
...
<script>
    ...
    function select_update_data() {
        var checkboxs = $("#table-prod tbody input[type=checkbox]:checked");
        for (var s = 0; s < checkboxs.length; s++) {
            checkboxs[s].click();
            checkboxs[s].checked = true;
        }

    }

    function select_product(id, kode, name, uom, qty, isChecked) {

        var qtyReal = qty != undefined ? qty : 0;
    ...
    function confirm_select() {
        ...
        for (var i = 0; i < arrData.length; i++) {
            ...
            html += '<td><input type="number" class="form-control " name="quantity[]" required value="' + arrData[i]['qty'] + '" min="0" step="any"></td>';
    ...
    function delete_product(i, id) {
        ...
        // document.getElementById('select_count').innerHTML = arrData.length - 1;
