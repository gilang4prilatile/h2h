<div class="modal fade" id="pilih-produk-modal" tabindex="-1" aria-labelledby="addIntegration" tabindex="-1">
          ...
          <tbody class="fw-bold">
            ...
            <tr>
              <td class="w-50px text-right">
                <div class="form-check text-right">
                  <input class="form-check-input" type="checkbox" value="{{$row->id}}" {{ in_array($row->id, $data_id_gc) ? 'checked' : '' }} onchange="select_product('{{ $row->id }}', '{{ $row->kode_barang }}', '{{ $row->name }}', '{{ $row->uom }}', {{ $data_qty_gc[$row->id] ?? 0 }} , this.checked)">
                </div>
              </td>
