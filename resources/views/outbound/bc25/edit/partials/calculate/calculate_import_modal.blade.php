        ...
        <div class="modal-content rounded">
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <div class="form-group row" style="margin-top: 10px;">
                    ...
                    {!! Form::select('hs_code_id', $hsCodes, null, ['class' => 'form-select hs-code-options-import', 'id' => 'dhscode_id_import', 'data-control' => 'select2']) !!}
                </div>
                <div class="form-group row" style="margin-top:10px;">
                    <div class="col-md-12">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-4">
                        ...
                        {!! Form::number('nett_weight', 0, ['class' => 'form-control', 'id' => 'dnett_weight_import', 'required' => true,  'maxlength' => '18', 'min' => '0', 'step' => 'any', 'oninput' => "if(this.value.length > this.maxLength){this.value = this.value.slice(0, this.maxLength)}
                        if(this.value<0){this.value='0'}"]) !!}
                    </div>
                </div>
                ...
                <div class="form-group row" style="margin-top: 10px;">
                    <div class="col-md-6">
                        ...
                        {!! Form::number('volume', 0, ['class' => 'form-control', 'id' => 'dvolume_import', 'required' => true,  'maxlength' => '18', 'min' => '0' , 'step' => 'any' ,  'oninput' => "if(this.value.length > this.maxLength){this.value = this.value.slice(0, this.maxLength)}
                        if(this.value<0){this.value='0'}"]) !!}
                    </div>
                    {{-- <div class="col-md-4">
                        <label class="form-label required">Bruto</label>
                        {!! Form::number('details[bruto]', 0, ['class' => 'form-control', 'id' => 'dbruto_import', 'required' => true,  'maxlength' => '18', 'min' => '0','step' => 'any', 'oninput' => "if(this.value.length > this.maxLength){this.value = this.value.slice(0, this.maxLength)}
                        if(this.value<0){this.value='0'}"]) !!}
                    </div> --}}
                    <div class="col-md-6">
                        <label class="form-label required">Nilai CIF</label>
                        {!! Form::number('details[nilai_cif]', 0, ['class' => 'form-control', 'id' => 'dnilai_cif_import', 'required' => true,  'maxlength' => '18', 'min' => '0','step' => 'any', 'oninput' => "if(this.value.length > this.maxLength){this.value = this.value.slice(0, this.maxLength)}
                        if(this.value<0){this.value='0'}"]) !!}
                    </div>
                </div>
                <div class="form-group row" style="margin-top: 10px;">
                    <div class="col-md-12">
                        ...
                        {!! Form::number('details[cif_rp]', 0, ['class' => 'form-control bg-secondary', 'id' => 'dcif_rp_import', 'required' => true,  'maxlength' => '18', 'min' => '0', 'readonly' ,'step' => 'any', 'oninput' => "if(this.value.length > this.maxLength){this.value = this.value.slice(0, this.maxLength)}
                        if(this.value<0){this.value='0'}"]) !!}
                    </div>
                </div>
                {{-- B1 --}}
                @include("outbound.bc25.edit.partials.calculate.calculate_tarif_fasilitas", ['from' => 'import'])

                <!--begin::Actions-->
                <div class="text-center" style="margin-top:20px;">
                    <button type="button" id="button_cancel_modal_tambah_barang_import" class="btn btn-light me-3">Cancel</button>
                    <button type="submit" id="button_tambah_barang_import" class="btn btn-primary">
                        <div class="spinner-border text-light" role="status" id="loading_button_tambah_barang_import" style="display: none;">
                        </div>
                        <span class="indicator-label" id="text_button_tambah_barang_import">Submit</span>
                    </button>
                </div>
                <!--end::Actions-->
            </form>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#add-form-calculate-import').submit(function(e){
                ...
                $('#button_tambah_barang_import').attr('type', 'button');
                $('#button_tambah_barang_import').attr("disabled", true);
                $('#loading_button_tambah_barang_import').show();
                $('#text_button_tambah_barang_import').hide();
                ...
                let method = 'POST'
                if (selectedImportGood.id) {
                    url += '/' + selectedImportGood.id
                    method = 'PUT'
                    selectedImportGoods = selectedImportGoods.filter(function(val) { return val.id != selectedImportGood.id })
                }
         
                selectedImportGoods.push(jsonObject)
                ...
                $.each(selectedGoods, (i, vi) => {
                    ...
                    if(vi.good_id == jsonObject.good_conversion_id){
                        ...
                        jsonObject.bc_form_id = selectedImportGood.bc_form_id;
                    }
                    ...
                    success: function(res) {
                        ...
                        if(res.status == 'success'){
                            ...
                            $.each(selectedImportGoods, (z, vz) => {
                                if(vz.good_id == jsonObject.good_conversion_id){

                                    vz.id = res.id;
                                    vz.hs_code_id = res.hs_code_id;
                            ...
                            $('#button_tambah_barang_import').attr('type', 'submit');
                            $('#button_tambah_barang_import').attr("disabled", false);
                            $('#loading_button_tambah_barang_import').hide();
                            $('#text_button_tambah_barang_import').show();
            ...
            const autoFillDetail = (good, details) => {
                ...
                $('#dhscode_id_import').val(good.hs_code_id).trigger('change')
            }
            ...
            const checkPungutanValue = () => {
                ...
                if(bm > 100 && bmType == 1 || bmValue > 100 && bmType == 1){
