                        ...
                        <div class="bg-transparent border-dashed border-1  rounded p-3" style="margin-bottom:10px;">
                            <div class="form-group row">
                                ...
                                <div class="col-md-7" id="inputan_fasilitas_barang" name="inputan_fasilitas_barang" style="display: none;">
                                    ...
                                    <input type="text" class="barang-terpilih" id="barang_terpilih[]" name="barang_terpilih[]">
                                    </div>
                                </div>
                                <div class="col-md-1">
...
$(document).ready(function() {
    ...
    function loadBarangDataIntoModal(documentId, rowIndex) {
        ...
        console.log('loadBarangDataIntoModal called for document ID:', documentId, 'Row Index:', rowIndex);
        ...
        var selectedGoodsMap = selectedGoods.reduce(function(map, good) {
            map[good.good_id] = true;
            return map;
        }, {});
    ...
    function saveSelectedBarangData(documentId, rowIndex) {
        ...
        console.log('Selected Barang Names:', selectedBarangNames);
        $currentRow.find('input.barang-terpilih').val(JSON.stringify(selectedBarang));

        // Find the correct row and update its display
        var $selectedFasilitasBarang = $currentRow.find('#selected_fasilitas_barang');
        $selectedFasilitasBarang.empty();
        selectedBarang.forEach(function(good) {
            $selectedFasilitasBarang.append('<div>' + good.serial_no + ' - ' + good.details.kode_barang + ' (' + good.name + ')</div>');
        });
    }
});
