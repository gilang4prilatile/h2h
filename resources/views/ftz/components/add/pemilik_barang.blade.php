        ...
        <div class="modal-content rounded">
            ...
            <div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
                ...
                <form action="#" id="add-form-pemilik-barang">
                    ...
                    {!! Form::select('pemilik_id', [], null, ['class' => 'form-select mt-3', 'data-control' => 'select2', 'id'=>'pemilik_id']) !!}
                    ...
                    <div class="card p-1" style="margin-top: 10px;" id="good_facility_select">
                        <div class="form-group">
                            <label class="form-label">Barang Fasilitas</label>
                            {!! Form::select('good_facility[]',[], null, ['class' => 'form-select', 'multiple' => 'true', 'data-control' => 'select2',  'id'=>'good_facility']) !!}
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $('#add-form-pemilik-barang').submit(function(e){
            ...
            if(jsonObject.pemilik_id != ""){
                $.ajax({
                    type: "GET",
                    url: '{{url('/')}}/master-data/profile/detail/'+jsonObject.pemilik_id,
                    success: function(res) { 

                        jsonObject['pemilik_tipe_identitas'] = res['tipe_identitas'];
                        jsonObject['pemilik_nomor_identitas'] = res['nomor_identitas'];
                        jsonObject['pemilik_nama'] = res['name'];
                        jsonObject['pemilik_alamat'] = res['address'];

                        if(jsonObject.no_seri != null 
                        && jsonObject.no_seri != undefined){
                            banyakPemilikBarang = banyakPemilikBarang.filter(function(val) { return val.no_seri != jsonObject.no_seri })
                        }
                        if(jsonObject.no_seri != '') banyakPemilikBarang.push(jsonObject);
                ...
                $.ajax({
                    type: "POST",
                    url: `{{url('/')}}/master-data/profile/create-ajax`,
                    headers: {'X-CSRF-TOKEN': jsonObject._token},
                    data: jsonObject,
                    success: function(result) {
                        if(result.status == 'success'){
                            jsonObject['pemilik_id'] = result.data.pemilik;
                            if(jsonObject.no_seri != null 
                            && jsonObject.no_seri != undefined){
                                banyakPemilikBarang = banyakPemilikBarang.filter(function(val) { return val.no_seri != jsonObject.no_seri })
                            }
        ...
        $('#pemilik-barang-modal').on('hide.bs.modal', function(e) {
            pemilikBarang = {};
        });
